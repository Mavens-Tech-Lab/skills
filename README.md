...
| **[shopify-upgrade](./shopify-upgrade/)** | Moves a codebase to a newer Shopify API version — any language, every surface (Admin GraphQL/REST, Storefront, Customer Account, Functions, App Bridge, extensions, webhooks, Liquid, Hydrogen). Finds the versions you actually pin (usually more than one), checks every call site against the **live** schema and changelog, shows you a risk-tiered plan **before editing anything**, then flips the version string last. |
