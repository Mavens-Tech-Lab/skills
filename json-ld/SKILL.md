...
- **Repair before replace.** Existing markup (JSON-LD, microdata, RDFa, or an SEO plugin's output) is audited, not blindly overwritten. Fix wrong values in place; convert formats or bypass a plugin only with the user's explicit OK from intake. Never let the page emit two conflicting nodes for the same entity. And existing markup is not grandfathered: parsing cleanly is not vocabulary-correctness — every property of every kept node is verified against the fetched definitions, same as anything newly emitted.
